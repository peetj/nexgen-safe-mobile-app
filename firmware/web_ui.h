...
<body>
  ...
  <div id="settingsModal" class="modal" hidden>
    <div class="modal-card">
      ...
      <div class="settings-block">
        ...
        <div id="safeNameStatus" class="mini-hint">Changes save automatically when you leave this field.</div>
  ...
  <div id="customThemeModal" class="modal" hidden>
    <div class="modal-card">
      <div class="modal-head">
        <div>
          <div class="modal-title">Custom Theme</div>
          <div class="hint">Color changes save automatically on this phone or browser.</div>
        </div>
        <button id="themeBackBtn" class="ghost small">Back</button>
      </div>
      <div class="settings-block first">
        <div class="settings-label">Theme name</div>
        <input id="customThemeName" maxlength="24" placeholder="My Safe Theme" value="">
        <div class="mini-hint">This name is just for the browser theme. It does not rename the safe itself.</div>
      </div>
      <div class="settings-block">
    ...
    <div class="modal-card compact">
      <div class="modal-head">
        <div>
          ...
          <div class="hint">The safe name changed. Restart the ESP32 to apply the new Wi-Fi and local hostname.</div>
        </div>
      </div>
      <p id="restartText" class="restart-copy"></p>
      <div class="footer-row">
        <button id="restartLaterBtn" class="ghost">Later</button>
        <button id="restartNowBtn" class="primary">Restart Now</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    const customThemeModalEl = document.getElementById('customThemeModal');
    ...
    function normalizeStoredThemeId(rawValue) {
      if (rawValue === CUSTOM_THEME_ID) return CUSTOM_THEME_ID;
      return PRESET_THEMES[rawValue] ? rawValue : 'ember';
    }

    function loadCustomTheme() {
      try {
        const parsed = JSON.parse(localStorage.getItem(CUSTOM_THEME_STORAGE_KEY) || 'null');
        if (!parsed || typeof parsed !== 'object') return null;
        return {
          name: (parsed.name || DEFAULT_CUSTOM_THEME_NAME).toString().trim().slice(0, 24) || DEFAULT_CUSTOM_THEME_NAME,
          values: normalizeThemeValues(parsed.values)
        };
      } catch (_) {
        return null;
      }
    ...
    function ensureCustomTheme(seedValues) {
      if (!customTheme) {
        customTheme = {
          name: DEFAULT_CUSTOM_THEME_NAME,
          values: normalizeThemeValues(seedValues || PRESET_THEMES.ember.values)
        };
        persistCustomTheme();
      }
      return customTheme;
    ...
    function getCurrentThemeValues() {
      if (activeThemeId === CUSTOM_THEME_ID) {
        return ensureCustomTheme().values;
      }
      return PRESET_THEMES[activeThemeId].values;
    }
    ...
    function applyTheme(themeId, persist = true) {
      const nextThemeId = normalizeStoredThemeId(themeId);
      if (nextThemeId === CUSTOM_THEME_ID) {
        const theme = ensureCustomTheme(getCurrentThemeValues());
        customThemeNameEl.value = theme.name;
        setThemeEditorValues(theme.values);
        applyThemeValues(theme.values);
      } else {
        applyThemeValues(PRESET_THEMES[nextThemeId].values);
        if (customTheme) {
          customThemeNameEl.value = customTheme.name;
          setThemeEditorValues(customTheme.values);
        }
      }

      activeThemeId = nextThemeId;
      document.body.dataset.theme = nextThemeId;
      renderThemeOptions(nextThemeId);
      updateThemeSummary();
      if (persist) {
        localStorage.setItem(ACTIVE_THEME_STORAGE_KEY, nextThemeId);
      }
    }

    function loadTheme() {
      customTheme = loadCustomTheme();
      const storedTheme = normalizeStoredThemeId(localStorage.getItem(ACTIVE_THEME_STORAGE_KEY) || 'ember');
      if (storedTheme === CUSTOM_THEME_ID) {
        ensureCustomTheme(PRESET_THEMES.ember.values);
      }
      applyTheme(storedTheme, false);
    }

    function saveCustomThemeEdits() {
      const theme = ensureCustomTheme(getCurrentThemeValues());
      const requestedName = customThemeNameEl.value.trim().slice(0, 24);
      theme.name = requestedName || DEFAULT_CUSTOM_THEME_NAME;
      theme.values = normalizeThemeValues(getThemeEditorValues());
      customTheme = theme;
      activeThemeId = CUSTOM_THEME_ID;
      persistCustomTheme();
      applyThemeValues(theme.values);
      renderThemeOptions(CUSTOM_THEME_ID);
      updateThemeSummary();
      localStorage.setItem(ACTIVE_THEME_STORAGE_KEY, CUSTOM_THEME_ID);
    }
    ...
    async function openCustomThemeModal() {
      ...
      settingsModalEl.hidden = true;
      customThemeModalEl.hidden = false;
    }

    function closeCustomThemeModal() {
      customThemeModalEl.hidden = true;
      settingsModalEl.hidden = false;
      updateThemeSummary();
    }

    function openRestartPrompt(name, url) {
      restartTextEl.textContent = 'Saved as ' + name + '. After restart, reconnect to ' + name + ' and open ' + (url || fallbackUrl) + '.';
      restartModalEl.hidden = false;
    }

    function closeRestartPrompt() {
      restartModalEl.hidden = true;
    }
    ...
    async function saveSafeNameIfNeeded() {
      markSafeNameDirty();
      if (!safeNameDirty) return true;

      const requestedName = safeNameEl.value.trim();
      if (!requestedName) {
        playError();
        safeNameStatusEl.textContent = 'Safe name cannot be empty.';
        return false;
      }

      try {
        const json = await api('/rename', { name: requestedName });
        const savedName = json.name || requestedName;
        desiredSafeName = savedName;
        ...
        if (json.restart_required) {
          ...
          setMessage('Safe name saved. Restart required to apply the new Wi-Fi name.');
          openRestartPrompt(savedName, desiredSafeUrl);
        } else {
          safeNameStatusEl.textContent = 'Safe name saved.';
          setMessage('Safe name updated to ' + savedName);
        }
        return true;
      } catch (err) {
        playError();
        safeNameStatusEl.textContent = 'Safe name save failed.';
    ...
    function syncAccessDetails(url, name, password) {
      ...
      if (!desiredSafeName) {
        lastSavedSafeName = effectiveName;
      }
    ...
    document.getElementById('settingsCloseBtn').onclick = async () => {
      playTap();
      await closeSettings();
    };

    themeSelectEl.onchange = async () => {
      playTap();
      if (themeSelectEl.value === CUSTOM_THEME_ID) {
    ...
    document.getElementById('restartLaterBtn').onclick = () => {
      playTap();
      closeRestartPrompt();
    };
    ...
    customThemeNameEl.addEventListener('input', () => {
    ...
    safeNameEl.addEventListener('input', () => {
      markSafeNameDirty();
      safeNameStatusEl.textContent = safeNameDirty
        ? 'Saving when you leave this field.'
        : 'Changes save automatically when you leave this field.';
    });
    ...
    customThemeModalEl.onclick = (event) => {
      if (event.target === customThemeModalEl) {
        closeCustomThemeModal();
      }
    };
